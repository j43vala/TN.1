...
const char* AP_SSID = "YourAP_SSID";
const char* AP_PASS = "12345";
IPAddress apIP(192, 168, 4, 1);
IPAddress subnet(255, 255, 255, 0);

String ipaddress = "0.0.0.0"; // Default IP
...
void setup() {
  ...
  pinMode(led1_led, OUTPUT);
  pinMode(led2_led, OUTPUT);
...
void loop() {
  if (settingMode) {
    ...
    if (mode == "T") {
      digitalWrite(led1_led, HIGH);
      delay(10);
      loopNormal();  // Transmitter mode loop
    } else if (mode == "R") {
      loopReceive();  // Receiver mode loop
      digitalWrite(led2_led, HIGH);
      delay(10);
      mb.task();
      delay(10);
    }
