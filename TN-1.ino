...
String macAddress;
String dyhostname;
String networkHostName = "wzero-";
String AP_SSID = "wzero_";
...
void setup() {
  ...
  dyhostname = generateDynamicHostname(AP_SSID);
...
String dynamicName(String mode, int deviceID, String AP_SSID, String setNumber) {

  Serial.println("Calling dynamicName...");
  if (mode == "R") {
    String receiver = String(AP_SSID) + String(setNumber) + "-R";
    Serial.println("Result for mode 'R': " + receiver );
    Serial.println("Calling dynamicName.R..");

    return receiver;
  } else {
    String transmitter =String(AP_SSID) + String(setNumber) + "-T-" + String(deviceID);
    Serial.println("Result for mode 'T': " + transmitter);
    Serial.println("Calling dynamicName.T..");
    return transmitter;
  }
}


String generateDynamicHostname(String AP_SSID) {
    uint8_t mac[6];
    WiFi.macAddress(mac); // Get the MAC address

    // Create a string to hold the MAC address
    String macAddress = "";
    for (int i = 0; i < 6; i++) {
        if (i > 0) {
            macAddress += ":"; // Add colon separator
        }
    ...
    // Combine AP SSID and MAC address to create the dynamic hostname
    String networkHostName = AP_SSID+ "TN.1-" + macAddress;

    // Print both MAC address and dynamic hostname
    Serial.println("MAC Address: " + macAddress);
    Serial.println("Dynamic Hostname: " + networkHostName);
...
void setupAP() {
  readPreferences();
  String dynamicAccessPoint = dynamicName(mode, deviceID, AP_SSID, setNumber);

  // WiFi.setHostname(dyhostname.c_str());  //Set the hostname to the desired value
  ...
  WiFi.softAP(dynamicAccessPoint, "");
  ...
  Serial.println("AP Mode Started. Connect to: " + dynamicAccessPoint);
